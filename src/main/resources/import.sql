...
Insert into usuario (username, rol_id, contraseña, correo, cliente_natural, bloqueado) values ('al1', 3, '$2a$10$PbD3OODWpcSLT/fEgpBp2ewPT.V1UyrEgq1cKlgD2lBD8gfJLsHDW','aerol1@mail.com',NULL,false);
Insert into usuario (username, rol_id, contraseña, correo, cliente_natural, bloqueado) values ('al2', 3, '$2a$10$jFOIUVg7qfSrvhkuTPrzb.KPr5eyAU.SNFKD92lrNJwuHNnw9axsS','aerol2@mail.com',NULL,false);
Insert into usuario (username, rol_id, contraseña, correo, cliente_natural, bloqueado) values ('al3', 3, '$2a$10$wzZa6oRsBQfTjFEQkCke1OpUPdtoLYVhaeIQyKb3SfKumLEFfVl4O','aerol3@mail.com',NULL,false);
...
create or replace function allAdminOfAerol (bigint)	returns table (id bigint,bloqueado boolean,cliente_natural boolean,contador_bloqueo integer,contraseña character varying(255),correo varchar(255),registro_completo boolean,username varchar(255),rol_id bigint ) language plpgsql as $$ begin return query select u.* from usuario u inner join rol r on u.rol_id = r.id where r.nombre='ROLE_aerolinea_admin' and u.id in (select adl.usuario_id from admin_aerolinea adl inner join aerolinea al on adl.aerolinea_id = al.id where al.id = $1 );end;$$
...
create or replace function findUserAdminOfAerol (bigint,bigint) returns table (id bigint,bloqueado boolean,cliente_natural boolean,contador_bloqueo integer,contraseña character varying(255),correo varchar(255),registro_completo boolean,username varchar(255),rol_id bigint) language plpgsql as $$ begin return query select * from usuario where usuario.id in (select adl.usuario_id from admin_aerolinea  adl where adl.usuario_id = $1 and adl.aeropuerto_id= $2);end;$$